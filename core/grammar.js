  ...
  rules: {
    ...
    close_parameter_option_value: ($) =>
      choice(
        ...
        seq(/del(ete)?/i, optional(seq(token.immediate('='), $.expression))),
      ),
    ...
    device_keywords: ($) => prec.right(choice(seq(token(seq('/', /[%A-Za-z][A-Za-z0-9]*/, token.immediate('='))), $.expression), $.expression, $.mnemonic_name)),
    open_parameters: ($) =>
      choice(
        seq(
         alias('(', $.bracket),
